...
- (void)layoutSubviews
{
    ...
    CGFloat inset = 20;
    NSUInteger count = self.emotions.count;
    CGFloat btnW = (self.width - 2 * inset) / JHEmotionMaxCols;
    CGFloat btnH = (self.height - inset) / JHEmotionMaxRows;
    for (int i = 0; i<count; i++) {
        UIButton *btn = self.subviews[i + 1];
        btn.width = btnW;
        btn.height = btnH;
        btn.x = inset + (i%JHEmotionMaxCols) * btnW;
        btn.y = inset + (i/JHEmotionMaxCols) * btnH;
    }
 ...
 *  @param btn 被点击的表情按钮
 */
- (void)btnClick:(JHEmotionButton *)btn
{
    // 显示popView
    [self.popView showFrom:btn];
